...
@st.cache_data  # 데이터를 처음으로 불러온 후에는 캐시에 저장 -> 빠르게 데이터 접근
def read_pensiondata():
    # print("▶ 공유파일 링크변환 경로명 : ", filepath)
    filepath = "https://drive.google.com/uc?id=1ccPdO5ZyHj24Sb6_3CzhkU4pmhe47TFZ"
    ...
    if company_name:
        ...
        if len(output) > 0:
            ...
            st.dataframe(df.loc[df['업종코드'] == info['업종코드'], ['사업장명', '월급여추정', '연간급여추정', '가입자수']]\
                .sort_values('연간급여추정', ascending=False).round(0), 
